...
def main():
    ...
    args.datadir = r'sdm_unips\results\20231128-01.data'
    ...
    for k in range(unit_vectors.shape[0]):
        ...
        nl, fd, fr = render(nml.to(device), l.to(device), base.to(device), rough.to(device), metallic.to(device), emit = 1.0, device = device)
