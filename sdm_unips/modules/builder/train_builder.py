        ...
        for iter in tqdm(range(self.train_epoch)):
            ...
            for batch_train in train_data_loader:                      
                ...
                if self.args.scalable:    
                    ...
                    for k in range(sliding_blocks):
                        ...
                        if torch.sum(patches_M[:, k, :, :, :]) > 0:
                            ...
                            if 'brdf' in self.args.target:
                                ...
                                nout = F.interpolate(nout, size=(patch_size, patch_size), mode='bilinear', align_corners=True)
                    ...
                    print(n_true.shape,nout.shape)
                    cos = np.sum(n_true.cpu().detach().numpy() * nout.cpu().detach().numpy(),-1)
                    cos = np.clip(cos,-1,1)
                    cos = np.arccos(cos)
                    angle = np.degrees(cos)
                    angle = angle.mean()
                    print('img_{}_mae:'.format(test_epoch),angle)
                    ...
                    self.writer.add_scalar('MAE', angle, iter)



                if test_epoch % self.save_model_epoch == 0:
                    save_models()
                ...
                if test_epoch % self.test_interval == 0:
                    ...
                    with torch.no_grad():       
                        ...
                        if 'normal' in self.args.target:
                            ...
                            cv2.imwrite(f'{data.data.data_workspace}/{test_epoch}_normal.png', 255*(0.5 * (1+nout[:,:,::-1])))                                      
                        ...
                        if 'brdf' in self.args.target:
                            ...
                            bout[r_s:r_e, c_s:c_e,:] = base
                            cv2.imwrite(f'{data.data.data_workspace}/{test_epoch}_baseColor.png', 255*bout[:,:,::-1])

                            rout = np.zeros((h_, w_), np.float32)
                            rout[r_s:r_e, c_s:c_e] = rough
                            cv2.imwrite(f'{data.data.data_workspace}/{test_epoch}_roughness.png', 255*rout[:,:])

                            mout = np.zeros((h_, w_), np.float32)
                            mout[r_s:r_e, c_s:c_e] = metal
                            cv2.imwrite(f'{data.data.data_workspace}/{test_epoch}_metallic.png', 255*mout[:,:])
                    self.net_brdf.train()
