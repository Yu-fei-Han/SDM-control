...
class dataloader():
    ...
    def load(self, objdir, image_prefix, light_suffix, margin = 0, max_image_resolution = 2048, aug=[]):
        ...
        directlist = []
        [directlist.append(p) for p in glob.glob(objdir + '/%s[!.txt]' % image_prefix, recursive=True) if os.path.isfile(p)]
        ...
        for i, indexofimage in enumerate(indexset):
            ...
            if i == 0:
                ...
                if os.path.isfile(nml_path):
                    ...
                    N = N * mask[:, :, np.newaxis]
                    mask_flag = True          
                # if ground truth base color map is avelable, generate base color-based mask
                if os.path.isfile(BC_path):
                    BC = cv2.cvtColor(cv2.imread(BC_path, flags = cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH), cv2.COLOR_BGR2RGB)
                    if BC.dtype == 'uint8':
                        bit_depth = 255.0
                    if BC.dtype == 'uint16':
                        bit_depth = 65535.0
                ...
                if os.path.isfile(metallic_path):
                    ...
                    if metallic.dtype == 'uint16':
                        bit_depth = 65535.0
                    metallic = np.float32(metallic)/bit_depth
        ...
        self.L = L
        self.a = BC
        self.r = roughness[...,None]
        self.m = metallic[...,None]
