...
class ScaleInvariantSpatialLightImageEncoder(nn.Module): # image feature encoder at canonical resolution
    def __init__(self, input_nc, use_efficient_attention=False):  # input_nc = 4 (RGB + mask)
    ...
    def forward(self, x, nImgArray, canonical_resolution):
        ...
        x = self.fusion(self.backbone(x_resized), nImgArray) #x_resized 10 4 256 256   x 10 256 64 64
        ...
        x = self.fusion(self.backbone(x_grid), nImgArray) # (K * N, C, canonical_resolution//4, canonical_resolution//4)  x_grid 40 4 256 256  x 40 256 64 64
        x = x.reshape(K, N, x.shape[1], canonical_resolution//4, canonical_resolution//4) # 4 10 256 64 64
        glc_grid = merge_tensor_spatial(x, method='tile_stride') # 10 256 128 128
        del x_grid
...
class Net(nn.Module):
    ...
    def forward(self, I, M, nImgArray, decoder_resolution, canonical_resolution):     
        ...
        glc = self.image_encoder(data, nImgArray, canonical_resolution) # torch.Size([N, 256, H/4, W/4]) [img, mask]
